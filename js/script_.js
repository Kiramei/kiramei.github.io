	...
	request1.onload = function() {
		if (request1.status === 200) {
			...
			for (let i = 0; i < news.length; i++) {
				$('.news_pane').append('<div class="news_content"><div class="title">' + news[i].title +
					'</div><div class="content">' + news[i].content + '</div><div class="date">' + news[i]
					.date1 + "</div></div>");
	...
	request.onload = function() {
		if (request.status === 200) {
			...
			for (let i = 0; i < musics.length; i++) {
				$('.list').append('<div class="mus"><div class="title">' + musics[i].name +
					'</div><a><div class="listen" index=' + i + '> Play</div></a><a href="' + musics[i].url +
					'" download><div class="down">Download</div></a></div>');
			}
		}
		addMovie();
		addPlay();
...
function addOption() {
	...
	$('.link:last-child').click(function() {
		$('.setting-menu').css('display', 'none');
		flag = 0;
	});
}
...
function addPause() {
	...
	p.on("click", function() {
		...
		if (flag === 0) {
			...
			audio.pause();
			$('.pause').html(
				'<svg t="1620388647690" fill="#0000009f" class="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" p-id="1637" width="22" height="22"><path d="M810.666667 512c0 14.933333-8.533333 27.733333-19.2 34.133333l0 0L273.066667 846.933333l0 0c-6.4 4.266667-12.8 6.4-19.2 6.4-21.333333 0-40.533333-17.066667-40.533333-40.533333L213.333333 211.2c0-21.333333 17.066667-40.533333 40.533333-40.533333 6.4 0 14.933333 2.133333 19.2 6.4l0 0 518.4 300.8 0 0C802.133333 484.266667 810.666667 497.066667 810.666667 512z" p-id="1638"></path></svg>'
			);
			flag = 0;
		}
	});
...
function addMouse() {
	...
	let linkElem = document.querySelectorAll("a,.rights,.movie-unit"),
		_i = 0;
